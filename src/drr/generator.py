...
def generate_record_sample(record: dict, crop_size: int = 128, volume_size: int = 64) -> dict:
    ...
    target = resize_volume_nearest(volume.mask == label, (volume_size, volume_size, volume_size))
    if not target.any():
    ...
    sample = _make_sample_from_target(record, target, crop_size)
    ...
    del volume, target, sample, batch
    gc.collect()
...
def _make_sample_from_target(record: dict, target: np.ndarray, crop_size: int) -> ReconstructionSample:
    ...
    for idx, category in enumerate(categories):
        image = _resize_square(project_volume_placeholder(target, angles.get(category, 0.0)), crop_size + 32)
